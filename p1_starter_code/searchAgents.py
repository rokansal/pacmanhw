...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        coords = state[0]
        seen = state[1]
        if coords in self.corners and not coords in seen:
            seen.append(coords)
        return (len(seen) == 4)
        util.raiseNotDefined()
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.EAST, Directions.WEST, Directions.NORTH, Directions.SOUTH]:
            ...
            if not hitsWall:
                newCoords = (nextx,nexty)
                newSeen = seen.copy()
                if newCoords in self.corners and newCoords not in newSeen:
                    newSeen.append(newCoords)
                successors.append(((newCoords,newSeen), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    for corner in corners:
        if corner not in state[1]:
            print(corner)
            costs.append(manhattanHeuristic(state[0], corner))
            print(costs)
    return max(costs)
...
def foodHeuristic(state, problem):
    ...
    for food in foodList:
        distances.append(sqrt((position[0] - food[0])**2 + (position[1] - food[1])**2))
